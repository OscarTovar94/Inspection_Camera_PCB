...
class CamaraVision:
    ...
    def crear_programa(self, nombre, cantidad_rois):
        ...
        config["PROGRAMA"] = {"nombre": nombre, "cantidad_rois": str(cantidad_rois)}
    ...
    def seleccionar_programa_inicial(self):
        ...
        dialogo.geometry("560x460")
        dialogo.resizable(False, False)
        dialogo.configure(fg_color=self.color_fondo)
        dialogo.transient(self.ventana)
        dialogo.grab_set()

        ctk.CTkLabel(dialogo, text="PROGRAMAS DE INSPECCIÓN", font=("Arial", 24, "bold")).pack(pady=(24, 6))
        ctk.CTkLabel(dialogo, text="Selecciona un programa existente o crea uno nuevo.", text_color="#AEB4D0").pack(pady=(0, 18))

        frame_sel = ctk.CTkFrame(dialogo, fg_color=self.color_panel)
        frame_sel.pack(fill="x", padx=28, pady=8)
        ctk.CTkLabel(frame_sel, text="Programa existente", font=("Arial", 14, "bold")).pack(anchor="w", padx=18, pady=(14, 6))
        programas = self.listar_programas()
        self.menu_programa_inicial = ctk.CTkOptionMenu(frame_sel, values=programas or ["-- Sin programas --"])
        ...
        ctk.CTkButton(frame_sel, text="ABRIR PROGRAMA", height=40, command=abrir_programa).pack(fill="x", padx=18, pady=(0, 6))
        ...
        def eliminar_programa():
            ...
            self.menu_programa_inicial.configure(values=nuevos or ["-- Sin programas --"])
            self.menu_programa_inicial.set(nuevos[0] if nuevos else "-- Sin programas --")
        ...
        ctk.CTkLabel(frame_nuevo, text="Crear nuevo programa", font=("Arial", 14, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", padx=18, pady=(14, 8))
        self.entry_nombre_programa = ctk.CTkEntry(frame_nuevo, placeholder_text="Ej. 428272")
        self.entry_nombre_programa.grid(row=1, column=0, padx=(18, 6), pady=6, sticky="ew")
        self.entry_cantidad_rois = ctk.CTkEntry(frame_nuevo, width=120, placeholder_text="ROIs")
        ...
        def crear_y_abrir():
            ...
            label_estado.configure(text=mensaje, text_color="#6FE3A1" if ok else "#FF6B6B")
            if ok:
                self.programa_actual = nombre.strip()
                seleccionado["ok"] = True
                dialogo.after(250, dialogo.destroy)

        ctk.CTkButton(frame_nuevo, text="CREAR Y ABRIR", height=40, fg_color="#218C4A", hover_color="#176A38", command=crear_y_abrir).grid(row=3, column=0, columnspan=2, padx=18, pady=(6, 14), sticky="ew")
    ...
    def cargar_programa(self, nombre_programa):
        ...
        self.total_rois = config.getint("PROGRAMA", "cantidad_rois", fallback=1)
        ...
        for indice, roi in enumerate(self.rois, start=1):
            ...
            if not config.has_section(seccion):
                continue
            roi["nombre"] = config.get(seccion, "nombre", fallback=roi["nombre"])
            ...
            roi["similitud_minima"] = config.getfloat(seccion, "similitud_minima", fallback=roi["similitud_minima"])
            roi["sensibilidad"] = config.getint(seccion, "sensibilidad", fallback=roi["sensibilidad"])
            roi["area_minima"] = config.getint(seccion, "area_minima", fallback=roi["area_minima"])
            roi["area_defecto_maxima"] = config.getfloat(seccion, "area_defecto_maxima", fallback=roi["area_defecto_maxima"])
            roi["porcentaje_diferente_maximo"] = config.getfloat(seccion, "porcentaje_diferente_maximo", fallback=roi["porcentaje_diferente_maximo"])
